...
def emit(area_data, rooms, mobs, objs, resets, helps, socials,
    ...
    w('# E/G/P limit: raw ROM reset-count field (cf. db.c reset_room): a value')
    w('# > 50 is a legacy encoding meaning limit 6; -1 means unlimited, and 0')
    w('# also means unlimited for E/G specifically (not P). Enforced at runtime.')
    w('# ("P", item_vnum, limit, container_vnum, max)         -- fill container in room up to max (capped by limit)')
    w('# ("R", room_vnum, num_dirs)                           -- shuffle first num_dirs exits (skipped on doors)')
