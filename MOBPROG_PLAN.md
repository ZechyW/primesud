...
> **Progress -- Phase E DONE (10/07/2026).** Final phase complete; the port is
> finished. New this phase:
> - **act trigger** (`mobprog.MOBtrigger` latch + `handler._act_trigger_mobs`):
>   `handler.act()` now fires TRIG_ACT on the NPC recipients of an act (1stMud
>   does this in perform_act's per-recipient loop, comm.c:2041; PrimeSUD's
>   player-only delivery never visits mobs, so it is a separate room-mob pass).
>   Recipient selection mirrors act_new (TO_CHAR/TO_VICT/TO_ROOM/TO_NOTVICT).
> - **MOBtrigger latch:** global in `mobprog.py`. [PRIMESUD] held off for the
>   *whole* act-trigger dispatch (stricter than 1stMud, which latches only
>   emote/asound) -- a hard recursion bound for the Prime's small stack. Wired
>   faithfully into `do_emote` and `_mp_asound` (the two 1stMud latch sites),
>   both of which had documented "not ported" notes now resolved.
> - **exit/exall triggers** (`mobprog.exit_trigger`, wired at the top of
>   `movement.move_char` for player movers): a room mob may react to and abort a
>   departure. exit gates on default_pos + can_see; exall fires regardless.
>   Phrase = door number (0=N..5=D). Resolved the move_char [Verified] TODO.
> - **speech recursion guard corrected:** the residual mob-to-mob speech gap
>   this plan attributed to the latch is actually closed by 1stMud's `do_say`
>   `!IsNPC(ch)` gate (act_comm.c:371) -- speech triggers fire only for a player
>   speaker. `do_say` now gates accordingly; a mob's prog `say` fires none.
> - Tests: 10 new in `tests/test_mobprog.py` (act fire/no-match/actor-exclude,
>   emote latch, latch bounds second-order act, mob-speaker fires no speech,
>   exit fire+abort/wrong-dir/pos+sight gate, exall). Full suite green (832).
> - Device checklist still pending on-calculator (act-heavy room idle CPU: the
>   act path now walks room mobs per `act()`, gated by `has_trigger`; + heap).
>
...
> [10/07/2026] All phases (A-E) done and harvested to DESIGN.md; TODO.md
> bullets struck. This file + OPUS_HANDOFF.md are kept committed for now at
> the repo owner's request rather than deleted per the usual plan lifecycle.
...
- **E -- act trigger + exit/exall. DONE 10/07/2026** (see the top progress
  note). act trigger fires on the NPC recipients of `handler.act()`; the
  global `MOBtrigger` latch is held off for the whole act-trigger dispatch
  ([PRIMESUD], stricter than 1stMud's emote/asound-only latching -- a hard
  recursion bound). exit/exall wired at the top of `move_char` for player
  movers. The speech mutual-recursion gap flagged here turned out to be
  closed by 1stMud's own `do_say` `!IsNPC(ch)` gate (speech fires only for a
  player speaker), not the latch -- `do_say` now gates accordingly.
