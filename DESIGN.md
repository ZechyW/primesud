...
| MOBprograms | Ported (10/07/2026, `mobprog.py`): **MOB** progs only -- obj/room progs skipped (the converter emits neither). Trigger set is all converter words except `surr` (surrender mechanic unported); the `act` + `exit`/`exall` triggers and the global MOBtrigger re-entrancy latch are deferred -- no content needs them yet (Phase E; full plan in git history via `MOBPROG_PLAN.md`). mp-command set is the solo-relevant subset; the group/mass commands (`mpgtransfer`/`mpgforce`/`mpvforce`) are skipped as logged no-ops. Any non-control prog line dispatches through the real command interpreter as the mob (NPC-safe verbs only, per the `mobprog.py` prog-safety docstring). First demo content: the Mud School acolyte greet/give/delay prog (`area_school`, authored via `patch_1stmud_deltas.py`) |
