...
level band. `gear.bin` (binary, layout in `tools/build_mob_index.py`, dumped
by `tools/dump_gear_bin.py`) stores canonical-score components plus one
fixed-width 30-byte record per static loot, ordinary-shop, floor, or
floor-container source, grouped by wear slot: loot records first, then
non-loot, each region sorted by a precomputed max-proficiency score bound,
descending, so the scan ends a region at the first record that cannot beat
the owned baseline. Display strings live deduplicated in a trailing string
table and are only read for winners. Both re-shapes exist because full-file
text scans cost 10+ s on-device: any per-row `split()` costs ~10 allocations
at ~0.5 ms each at full heap, so the scanner rejects records with raw byte
arithmetic (zero allocations) instead of parsing text. `recommend` scans
only these files and resident player/static tables, retains bounded
displayed results, creates no candidate instances, loads no area, and keeps
no parsed cache. `mobs.idx` keeps its original six-field schema for
non-recommendation consumers.
