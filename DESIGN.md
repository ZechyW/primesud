...
| AC types (Pierce/Bash/Slash/Exotic) | **Partially ported** | 1stMud `one_hit` (`fight.c`) selects one of four `armor[]` buckets (pierce/bash/slash/exotic) based on `dam_class`; `GetArmor` then adds the DEX defensive bonus. PrimeSUD now stores bucket AC on actors/items and uses bucket selection in `one_hit`, but broader combat fidelity work around damage flow, defenses, and res/imm/vuln remains incomplete |
...
| Clan / rank | **Not ported** | Multiplayer concepts |
| Trivia economy | **Deferred** | Point counter and trivia pill are retained for quest compatibility; full trivia shop/reward system comes later |
...
| Pulse timing | `PULSE_VIOLENCE = 3xPPS`, `PULSE_MOBILE = 4xPPS`, `PULSE_TICK = 45xPPS` | **`2xPPS`, `5xPPS`, `30xPPS`** | Faster combat and regen ticks for single-player UX; slower mob wander |
