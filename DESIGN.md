...
static loot, ordinary-shop, floor, or floor-container source, grouped into
per-wear-slot segments behind a byte-length directory line; inside each
segment, `@min_level|bytes` headers open 5-level item bands (ascending)
whose rows sort by max-proficiency score bound (descending), so the scan
breaks at the first band above the player's level and jumps past a band once
its bound cannot beat the owned baseline. Both re-shapes exist because the
original full-file scans cost 10+ s on-device: per-row `split()` allocation
(~0.5 ms per alloc at full heap) dominated, so the index layout now does the
filtering that the scan used to. `recommend` scans only these files and
resident player/static tables, retains bounded displayed results, creates no
candidate instances, loads no area, and keeps no parsed cache. `mobs.idx`
keeps its original six-field schema for non-recommendation consumers.
