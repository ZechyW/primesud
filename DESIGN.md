...
Stock data contains exactly one cross-area reset cluster (audited 13/07/2026): `arachnos` spawns the Queen Spider and three poisonous spiders (plus G-gear) into Haon Dor room 6134. Resets that merely *spawn a foreign template in the owner's own room* are not cross-area -- the entry partitions normally and the template cascade-loads via the `MOB_DEFS`/`ITEM_DEFS` LazyDicts.
