...
| Room light counter | `room->light` bumped incrementally in `char_to_room`/`equip_char` | Computed on demand: `room_light(vnum)` counts lit light-slot items among the room's occupants | The many extraction/removal paths would drift a persistent counter and force it into the save |
