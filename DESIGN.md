...
- **Save-path caches.** `_PENDING_VNUM_CACHE` (per-room pending-token vnum scans, validated by raw-string identity -- pending strings are replaced wholesale, never mutated) and `_SNAP_ENC_CACHE` (vnum -> encoded `it.` line, valid while the stamped revision matches; prefilled from raw save bytes at load). Both owned by world.py, cleared in `reset_lazy`. Device-measured: full-world save 11.7s -> 1.6s steady-state (debug/save_smoke-1/-2.log, 29/07/2026).
