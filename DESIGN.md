...
and mobs.bin records. Test fixtures build chars via `_char_base()` +
...
| Attack-spell damage | Seven spells (magic missile, burning hands, chill touch, color spray, fireball, lightning bolt, shocking grasp) roll `number_range((level\|50)/2, (level\|50)*2)` -- one flat ~50-63 band from level 1 | ROM 2.4 stock `dam_each` tables restored (`magic.py`), each spell with its own curve and level gate | 1stMud's `(level\|50)` rewrite (bitwise OR, likely a `+` typo) buffed magic missile ~7x and made every attack spell outdamage weapons from level 1; the ROM tables recover the intended per-spell balance (docs/FIXES.md) |
...
| Remort gold fee | `do_remort` gates on `check_worth(ch, 500000, VALUE_GOLD)` and `finish_remort` spends it with `deduct_cost(..., VALUE_GOLD)` -- carried gold only, so the 500k has to be withdrawn first | Eligibility tests carried + banked gold together, and `_pay_remort_gold` (training.py) spends carried coins first, then the bank balance | The withdrawal trip was pure ceremony: coin weight only ever gated pickups, never the remort itself (upstream players just hauled the 500k over-encumbered). Silver stays excluded, matching upstream `VALUE_GOLD`; bank itself reopened in Midgaard (WORLD.md) |
...
| Chessboard piece placement | Only kings and queens are sentinel; other pieces wander randomly | All 32 chess pieces are sentinel; Caliph Omar's ghost remains mobile | Keeps reset-authored pieces on the canonical squares described by each room; sentinel affects idle wandering only, so combat fleeing remains unchanged |
...
| `look <target>` vs `examine <target>` | Two commands: help.dat says "EXAMINE is short for 'LOOK container' followed by 'LOOK IN container'", so `look` shows the description and `examine` adds coin counts / container contents | Merged: typed `examine <target>` is literally `do_look(player, [target])`, and do_look's object path emits the extras itself (`_examine_extras`) | Two near-identical lookup commands with a subtle delta is bad UX for one player with no muscle memory to protect. Merging also kills the re-resolution mismatch: extras now attach to the exact instance do_look's scan matched, where upstream (and our old `examine` tail) re-resolved the argument through `get_obj_here`, which numbers each list separately and so could miss or land on a different object. Remaining deltas: with no argument `look` shows the room while `examine` opens the [PRIMESUD] picker, and the `look in` / `look on` subsyntax stays look-only |
...
Single ROM 2.4 (QuickMUD-dialect) pipeline: `areas/*.are` are editable working copies (pristine upstream originals kept under `reference/`), converted by the sole `tools/are_to_primesud.py`. The old 1stMud-format converter is retired; 1stMud-only areas (`limbo`, `quest`, `pestates`) were converted once to ROM 2.4 so every area shares one format and one converter. `#SPECIALS`/`#SHOPS` are baked into each mob's `MOBILES` entry (`spec_fun`/`shop` keys) at conversion time rather than merged at load; a special/shop referencing a mob vnum outside its own file is a hard conversion error. A handful of 1stMud flag-bit extensions absent from stock QuickMUD `merc.h` (item `extra_flags` bits 17/26, room flag bits 4/5/20-22) are treated as canonical since the runtime is 1stMud-ported. Generated data files (`area_*.txt`, `help.txt`) use `.txt` rather than `.dat` so tooling doesn't mistake them for binary; generated plain-text index files (`help.idx`, `socials.idx`, `paths.idx`) use `.idx`, binary ones (`mobs.bin`, `objs.bin`, `foes.bin`, `gear.bin`) use `.bin`. `mobs.bin` stores one record per template (vnum, level, home tag, keywords, short_descr, spawn tags): counters/debug use metadata without area loads, while ordered reset-owning spawn tags preserve unloaded-mob lookup priority. `objs.bin` likewise stores every object template (vnum, home tag, keywords, spawn tags); `debug vnum` uses vnum/home/keywords and `locate object` uses ordered `O`/`E`/`G`/`P` reset-owning tags. The converter fails loudly (`ValueError`) on any construct it doesn't handle -- unknown sections (`#AREADATA`, `#MOBOLD`/`#OBJOLD`), unrecognized trailer/reset/special letters, truncated payloads, `spec_fun` names missing from `src/special.py` `SPEC_TABLE` -- mirroring QuickMUD's own `bug()`+`exit(1)` loader; verified 2026-07-05 against all 53 stock QuickMUD areas (52 convert clean; `hood.are` correctly rejected for unimplemented specs).

---

## Lazy area loading

**Recommendation indexes (settled 31/07/2026; binary re-shape 02/08/2026).**
Both `recommend` indexes are binary (layouts beside the pack functions in
`tools/build_mob_index.py`; readable dumps via `tools/dump_gear_bin.py` /
`tools/dump_foes_bin.py`), because full-file text scans cost seconds
on-device: any per-row `split()` costs ~10 allocations at ~0.5 ms each at
full heap, so both scanners walk records with raw byte arithmetic -- zero
allocations per reject -- and resolve display strings for winners only, from
one read of a deduplicated trailing string table. `foes.bin` holds one
variable-length record (vnum, level, name ref, spawn-tag ids) per fightable
reset-backed mob, grouped into per-level segments behind per-level byte
sizes in the header, so `recommend mobs` seeks and reads only its level band
and ranks it in two passes (per-level counts for the band-floor raise, then
a packed-int sort key keeping the top 10). `gear.bin` stores
canonical-score components in one fixed-width 30-byte record per static
loot, ordinary-shop, floor, or floor-container source, grouped by wear slot:
loot records first, then non-loot, each region sorted by a precomputed
max-proficiency score bound, descending, so the scan ends a region at the
first record that cannot beat the owned baseline. `recommend` scans only
these files and resident player/static tables, retains bounded displayed
results, creates no candidate instances, loads no area, and keeps no parsed
cache. Device I/O caveat for all `.bin` readers:
`open(.., "rb")` returns str with character-counted sized reads (see
docs/BUILTINS.md sec. File open() binary-mode semantics); every read goes
through `recommend._as_bytes`.

**Keyword indexes (binary re-shape 02/08/2026).** The same argument
retired the last two hot text indexes: `mobs.bin` and `objs.bin` share one
format (`KX01`, layout beside `pack_key_index` in
`tools/build_mob_index.py`, readable dump via `tools/dump_key_bin.py`) --
fixed-field variable-length records, then one lowercased keyword blob,
then a deduplicated display-string table. `src/keyidx.py` is the shared
reader: name search is two-phase, sweeping the blob with native `find()`
for the target's first word (accepting only word-boundary hits, which are
exactly `is_name` prefix matches) and mapping hits back to records with
one sequential two-pointer walk, since records and blob entries share an
order. Nothing allocates per record; only the handful of confirmed
candidates get sliced and re-checked with `handler.is_name`. Record order
is a correctness invariant of the generator (mobs: cheapest spawning area
then vnum; objects: `_AREA_FILES` order then vnum), because tag ids are
`_AREA_FILES` positions -- sorting spawn-tag ids ascending *is* the
cheapest-area-first order `spell_portal`/`nexus`/`gate` walk. Consumers:
`_find_unloaded_mob`, `locate object`, `debug vnum`, and the mob
kill/death counters; all four now degrade quietly (empty result) on a
...
`path <area or mob>` (`src/path.py`) reports the same compressed route without moving. Area names take precedence, matching 1stMud. Loaded mobs use their live room; an absent mob is resolved through `mobs.bin` and the existing two-candidate area-load cap, so wandering and restored mob positions stay authoritative without adding spawn-room data to the index. Routing runs over a precomputed **border graph** (`src/paths.idx`, built offline by `tools/build_path_index.py`): S records give the shortest intra-area walk from each entry room to each exit room of an area, X records give every cross-area exit. `_route` Dijkstras over that graph (integer weights, O(V^2) linear-min -- `heapq` availability on-device unverified) plus two live BFS legs inside already-loaded areas: player room -> source-area exits (source area is always loaded) and, for mob targets, target-area entries -> the mob's room (that area was loaded by the mob lookup). Zero area loads at routing time, and routes are exact shortest paths -- the earlier corridor-restricted BFS over the static area-adjacency chain assumed any entry of an area reaches any exit, which real (internally partitioned) areas break constantly: measured on full data, 536/2256 area pairs got no route despite one existing and 30% of exact-room targets were unreachable. Step counts match an unrestricted full-world BFS (asserted against real data in `tests/test_path_realworld.py`); the chosen equal-length route may differ from upstream's. One deviation from upstream's "stop at the first room inside the target area" rule: V records flag *sliver* border rooms -- ones whose in-area BFS reaches less than half their area, i.e. a fragment cut off from the rest of it, such as New Thalos' river rooms 9772-9775, which only link Midgaard to Haon Dor. An area target never lands on a flagged room (so `run new thalos` walks to the West Gate instead of stranding the player on the river); a second Dijkstra pass accepts them only if the area is otherwise unreachable, and mob targets ignore the flag entirely. Rooms carrying an `R` reset (`mob._reset_randomize_exits`, `num_dirs >= 2`) have their exits Fisher-Yates shuffled at *every* area reset, so a compass direction out of one is stale within minutes -- the index used to freeze the build-time layout and `run` would dead-end mid-walk ("Alas, you cannot go that way"). Steps leaving such a room are therefore written as **room-target tokens** `*<vnum>` ("take whichever live exit leads to room `<vnum>`") in both S dirs strings and X records (whose `dir` field becomes `*`); the build tool derives the shuffled set from the loaded reset data, so no list is hardcoded. Sound because the shuffle permutes exit pointers *within* a room but preserves its neighbour set (and skips rooms with doors on shuffled slots), making both tokens and in-maze BFS distances shuffle-invariant. Tokens are atomic single steps: they never merge into direction runs, and a run count is never emitted straight after one (its digits would fuse with the token's vnum, so `*3054` + `3n` becomes `*3054nnn`). `_parse_run_buf` turns them into `goto` steps and `run_buf_step` resolves each against the current room's live exits, cancelling the run like blocked movement if nothing matches; `path` prints them as `?` plus one helper line while `player["last_path"]` keeps the raw tokens for `run`. The live BFS legs need no tokens -- they read live exits at compute time (a reset mid-walk can still invalidate them, as upstream). The upstream mob condition is inverted (`!= NULL` followed by failure clauses joined with `||`); PrimeSUD ports its clear gate-style intent instead: safe/private/solitary/no-recall rooms, quest/gquest mobs, targets 3+ levels higher, summon immunity, and a successful save all hide the destination. Arena/clan/area-access checks remain inapplicable until those systems exist. Because `path` does not move, it force-runs the normal far-area eviction check after the query.

`locate object` preserves 1stMud's world-global search without loading the full
world at once. It scans resident state first, then uses `objs.bin` reset-owning
