...
| Black Pawn's Halberd weapon class typo | `chess2.are` | obj 4224 | [PRIMESUD] -- corrects stock `polear` to canonical `polearm`; upstream's prefix lookup accepted the typo, but PrimeSUD requires an exact weapon-class name. Object sections have no comment seam, so provenance lives here |
| Sentinel chess pieces | `chess2.are` | mobs 4200-4207 | [PRIMESUD] -- adds sentinel to pawns, rooks, knights, and bishops so all 32 pieces remain on their reset-authored squares during idle movement; kings/queens 4208-4211 were already sentinel, while non-piece ghost 4212 remains mobile. The mobile format has no per-entry comment seam, so provenance lives here |
...
| Midgaard City Bank reopened | `midgaard.are` | rooms 3008 (name/desc, `F` ROOM_BANK flag), 3006 (desc + `D4` exit desc); objs 3140 (liquidation notice rewritten as reopening notice, keywords `notice reopening bank`), 3162 (map art label) | [PRIMESUD] -- stock's "The Defunct Reception" (ROM's lore site for the closed Midgaard City Bank) reopened as a working bank room (WORLD.md). Room/object sections have no comment seam, so provenance lives here; the 3140 `O` reset comment is updated in place |
| Reciprocal Dragon Tower Great Hall exit | `draconia.are` | room 2224 `w` -> 2223 | [PRIMESUD] -- stock has room 2223 `e` -> 2224 but no reverse exit or one-way mechanism; restores ordinary backtracking. Room sections have no comment seam, so provenance lives here |
