...
void add_to_wizlist(WizData * wiz)
{
	WizData *tmp;
...
Do_Fun(do_wizlist)
{
	...
	if (!NullStr(arg1) && (ch->level >= MAX_LEVEL - 1))
	{
		...
		if (!str_prefix(arg1, "email"))
		{
			...
			if ((argument = is_invalid_email(strip_color(argument))) != NULL)
			{
		...
		if (!str_prefix(arg1, "jobs"))
		{
			...
			if (curr)
				chprintlnf(ch, "%s's jobs are now %s.", curr->name,
						   curr->jobs);
			else
				chprintlnf(ch, "%s not found.", capitalize(arg2));
			return;
	...
	for (curr = wiz_first; curr != NULL; curr = curr->next)
	{
		chprintlnf(ch,
				   ...
				   curr->level, str_width(10, curr->name), curr->email,
				   curr->jobs);
		found = true;
	}
	if (!found)
		chprintln(ch,
				  "                   {WNo Immortals Found At This Time.{x");
	chprintln(ch, NULL);
	return;
}

void update_wizlist(CharData * ch, int level)
{
	WizData *next;
	WizData *curr;
	char jobs[MIL];
	...
	replace_str(&curr->email,
				GetStr(ch->pcdata->email, "-User Personal Board-"));
	replace_str(&curr->jobs, GetStr(jobs, "nothing yet"));
	curr->level = level;
	add_to_wizlist(curr);
	rw_wiz_data(act_write);
	return;
