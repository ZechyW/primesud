...
void gain_exp(CharData * ch, int gain)
{
									   ...
									   ch->pcdata->points) * (ch->level + 1))
	{
		...
		new_wiznet(ch, NULL, WIZ_LEVELS, true, 0, "$N has attained level %d!",
				   ch->level);
...
int mana_gain(CharData * ch)
{
	...
	{
		gain =
			(get_curr_stat(ch, STAT_WIS) + get_curr_stat(ch, STAT_INT) +
			 ch->level) / 2;
		number = number_percent();
...
void mobile_update(void)
{
	...
	for (ch = char_first; ch != NULL; ch = ch_next)
	{
			...
			&& ch->in_room->content_first != NULL && number_bits(6) == 0)
		{
			...
			max = 1;
			obj_best = 0;
			for (obj = ch->in_room->content_first; obj;
				 obj = obj->next_content)
			{
				if (CanWear(obj, ITEM_TAKE) && can_loot(ch, obj)
...
void char_update(void)
{
	...
	for (ch = char_first; ch != NULL; ch = ch_next)
	{
		...
		if (ch->position >= POS_STUNNED)
		{
			...
			if (IsNPC(ch) && ch->zone != NULL && ch->zone != ch->in_room->area
				&& ch->desc == NULL && ch->war == NULL && ch->fighting == NULL
				&& !IsAffected(ch, AFF_CHARM) && number_percent() < 5)
			{
		...
		if (!IsNPC(ch))
		{
			...
			{
				int pos = timer_table[i].timer;

				if (HasTimer(ch, pos))
				{
					if (timer_table[i].fTime
						? (current_time >=
						   GetTimer(ch, pos)) : UpdateTimer(ch, pos))
					{
						ClearTimer(ch, pos);
						chprintln(ch, timer_table[i].off_msg);
						switch (pos)
						{
							case TIMER_PK:
								RemBit(ch->act, PLR_PK);
								break;
							default:
								break;
						}
	...
	for (ch = player_first; ch != NULL; ch = ch_next)
	{
		...
		if (ch->desc != NULL
			&& (int) ch->desc->descriptor % 30 == save_number)
		{
			save_char_obj(ch);
		}
...
void aggr_update(void)
{
	...
	for (wch = player_first; wch != NULL; wch = wch_next)
	{
		...
		if (IsNPC(wch) || wch->level >= LEVEL_IMMORTAL || wch->in_room == NULL
		...
		for (ch = wch->in_room->person_first; ch != NULL; ch = ch_next)
		{
			...
			count = 0;
			victim = NULL;
			for (vch = wch->in_room->person_first; vch != NULL;
				 vch = vch_next)
			{
				...
				if (!IsNPC(vch) && vch->level < LEVEL_IMMORTAL
					...
					&& (!IsSet(ch->act, ACT_WIMPY) || !IsAwake(vch))
					&& can_see(ch, vch))
				{
...
void update_handler(void)
{
	...
	if (--pulse_area <= 0)
	{
		...
		strcpy(crash_info.logline, "pulse_area");
		area_update();
		bank_update();
	}
	...
	if (--pulse_music <= 0)
	{
		...
		strcpy(crash_info.logline, "pulse_music");
		song_update();
	}
	...
	if (--pulse_mobile <= 0)
	{
		...
		strcpy(crash_info.logline, "pulse_mobile");
		mobile_update();
	}
	...
	if (--pulse_violence <= 0)
	{
		...
		strcpy(crash_info.logline, "pulse_violence");
		violence_update();
	}
	...
	if (--pulse_point <= 0)
	{
		...
		strcpy(crash_info.logline, "pulse_point");
	...
	strcpy(crash_info.logline, "auction_update");
	auction_update();
	strcpy(crash_info.logline, "crs_update");
	crs_update();
	strcpy(crash_info.logline, "aggr_update");
	aggr_update();
