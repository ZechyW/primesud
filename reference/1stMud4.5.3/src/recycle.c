...
void free_descriptor(Descriptor * Data)
{
	if (!IsValid(Data))
		return;
...
void free_gen_data(GenData * Data)
{
	if (!IsValid(Data))
		return;
...
void free_affect(AffectData * Data)
{
	if (!IsValid(Data))
		return;
...
PcData *new_pcdata(void)
{
	...
	*Data = pcdata_zero;
	...
	memset(Data->timers, 0, MAX_TIMER);
	Data->timezone = -1;
	Data->prime_class = 0;
	Data->trivia = 0;
...
void free_pcdata(PcData * Data)
{
	...
	for (nl = Data->killer_first; nl != NULL; nl = nl_next)
	{
		nl_next = nl->next;
		free_namelist(nl);
	}
...
int bprint(Buffer * buffer, const char *string)
{
	int ret_len, len;
	char *oldstr;
...
int bprintf(Buffer * buffer, const char *messg, ...)
{
	char buf[MPL];
	va_list args;
...
int bprintlnf(Buffer * buffer, const char *messg, ...)
{
	va_list args;
	char send_buf[MPL];
...
int bprintln(Buffer * buffer, const char *messg)
{
	int len = 0;
...
void free_prog_list(ProgList * Data)
{
	if (!IsValid(Data))
		return;
...
void free_corpse(CorpseData * Data)
{
	if (!IsValid(Data))
		return;
...
void free_auction(AuctionData * Data)
{
	if (!IsValid(Data))
		return;
...
void free_wpwd(WebpassData * Data)
{
	if (!IsValid(Data))
		return;
...
void free_social(SocialData * Data)
{
	if (!IsValid(Data))
		return;
...
void free_deity(DeityData * Data)
{
	if (!IsValid(Data))
		return;
...
void free_reset(ResetData * Data)
{
	if (!IsValid(Data))
		return;
...
void free_room_index(RoomIndex * Data)
{
	int door;
	ExDescrData *pExtra;
	ResetData *pReset;
...
void free_obj_index(ObjIndex * Data)
{
	ExDescrData *pExtra;
	AffectData *pAf;
	ProgList *op;
...
void free_char_index(CharIndex * Data)
{
	ProgList *mp;
...
void free_warlist(WarData * Data)
{
	if (!IsValid(Data))
		return;
...
DisabledData *new_disabled(void)
{
	DisabledData *Data;
...
void free_disabled(DisabledData * Data)
{
	if (!IsValid(Data))
		return;
...
void free_note(NoteData * Data)
{
	...
	free_string(Data->text);
	free_string(Data->reply_text);
	free_string(Data->email_addr);
	free_string(Data->reply_addr);
	top_note--;
	Invalidate(Data);
...
NoteData *new_note(void)
{
	...
	*Data = note_zero;
	...
	Data->reply_addr = NULL;
	Validate(Data);
	return Data;
}

#ifndef DISABLE_WEBSRV
WebDescriptor *new_webdesc(void)
{
	WebDescriptor *Data;
...
void free_webdesc(WebDescriptor * Data)
{
	if (!IsValid(Data))
		return;
...
NameProfile *new_name_profile(void)
{
	NameProfile *Data;
	int i;
...
void free_name_profile(NameProfile * Data)
{
	int i;
...
void free_namelist(NameList * Data)
{
	if (!IsValid(Data))
		return;
...
ColorTemplate *new_color_template(void)
{
	ColorTemplate *Data;

	if (color_template_free != NULL)
	{
		Data = color_template_free;
		color_template_free = color_template_free->next;
		top_color_template_free--;
	}
	else
	{
		Data = (ColorTemplate *) alloc_perm(sizeof(ColorTemplate));
	}
	*Data = color_template_zero;
	top_color_template++;
	Data->name = &str_empty[0];
	Data->description = &str_empty[0];
	Data->colors = NULL;
...
void free_color_template(ColorTemplate * Data)
{
	if (!IsValid(Data))
		return;
	...
	top_color_template--;
	Invalidate(Data);
	LinkSingle(Data, color_template_free, next);
	top_color_template_free++;
}
