...
Proto(int find_path, (int, int, CharData *, int, bool));
...
void init_world(RoomIndex * room_db[])
{
	memset(room_db, 0, sizeof(RoomIndex *) * top_room_index);
}

CharData *get_char_area(CharData * ch, char *argument)
{
	char arg[MIL];
	CharData *ach;
	int number;
...
RoomIndex *room_find(RoomIndex * room_db[], int key)
{
...
int room_enter(RoomIndex * rb[], int key, RoomIndex * rm)
{
	RoomIndex *temp;
...
RoomIndex *room_find_or_create(RoomIndex * rb[], int key)
{
	RoomIndex *rv;
...
int room_remove(RoomIndex * rb[], int key)
{
	RoomIndex *tmp;
...
int find_path(int in_room_vnum, int out_room_vnum, CharData * ch, int depth,
			  bool in_zone)
{
				...
				if (exit_ok(exitp) && (thru_doors ? GO_OK_SMARTER : GO_OK))
				{
					...
					if (tmp_room != out_room_vnum)
					{
						if (!hash_find(&x_room, tmp_room) && (count < depth))
						{
							...
							res = hash_find(&x_room, q_head->room_nr);
							j = (i + 1);
					...
					else
					{
						...
						if (*(int *) (res = hash_find(&x_room, tmp_room)) ==
							-1)
						{
							if (x_room.buckets)
							{
...
Do_Fun(do_hunt)
{
	...
	if (direction == -1)
	{
		act("You couldn't find a path to $N from here.", ch, NULL, victim,
			TO_CHAR);
		return;
	}
...
void hunt_victim(CharData * ch)
{
	...
	if (ch->in_room == ch->hunting->in_room)
	{
		...
		{
			act(CTAG(_SAY1) "$n" CTAG(_SAY1) " glares at $N" CTAG(_SAY1)
				" and says, '" CTAG(_SAY2) "Ye shall DIE!" CTAG(_SAY1) "'{x",
				ch, NULL, ch->hunting, TO_NOTVICT);
			act(CTAG(_SAY1) "$n" CTAG(_SAY1) " glares at you and says, '"
				CTAG(_SAY2) "Ye shall DIE!" CTAG(_SAY1) "'{x", ch, NULL,
				ch->hunting, TO_VICT);
			act(CTAG(_SAY1) "You glare at $N" CTAG(_SAY1) " and say, '"
				CTAG(_SAY2) "Ye shall DIE!" CTAG(_SAY1) "'{x", ch, NULL,
				ch->hunting, TO_CHAR);
		}
		else
		{
			act(CTAG(_SAY1) "$n" CTAG(_SAY1) " glares at $N" CTAG(_SAY1)
				" and says, '" CTAG(_SAY2) "Hey, I remember you!" CTAG(_SAY1)
				"'{x", ch, NULL, ch->hunting, TO_NOTVICT);
			act(CTAG(_SAY1) "$n" CTAG(_SAY1) " glares at you and says, '"
				CTAG(_SAY2) "Hey, I remember you!" CTAG(_SAY1) "'{x", ch,
				NULL, ch->hunting, TO_VICT);
			act(CTAG(_SAY1) "You glare at $N" CTAG(_SAY1) " and say, '"
				CTAG(_SAY2) "Hey, I remember you!" CTAG(_SAY1) "'{x", ch,
				NULL, ch->hunting, TO_CHAR);
		}
