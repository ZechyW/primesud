...
Do_Fun(do_arena_bet)
{
	...
	chprintlnf(ch, "You have placed a %ld gold wager on %s", wager,
			   fighter->name);
	return;
}

void check_arena(CharData * ch, CharData * victim)
{
	CharData *wch;
	float odds;
	float lvl1, lvl2;
...
void extract_arena(CharData * ch)
{
...
{
	cmd_syntax(ch, NULL, n_fun, "challenge <name>|drop", "accept <name>",
			   "decline <name>", "bet <name>", NULL);
...
Do_Fun(do_arena)
{
	vinterpret(ch, n_fun, argument, "challenge", do_arena_challenge,
			   "decline", do_arena_decline, "accept", do_arena_accept, "bet",
			   do_arena_bet, "clear", do_arena_clear, "busy", do_arena_busy,
			   "lock", do_arena_lock, NULL, do_arena_help);
}
