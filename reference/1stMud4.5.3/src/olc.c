...
OlcTable olc_ed_table[ED_MAX] = {
	...
	ED_TABLE("SongEdit", "song", ED_SONG, songedit, song, name,
			 FIELD_STRING),
	ED_TABLE("ColEdit", "color", ED_COLOR, coledit, color, name,
			 FIELD_STRING),
};

OlcTable *olc_lookup(ed_t ed)
{
	int i;
...
bool olc_show(CharData * ch, OlcTable * ed)
{
	Buffer *output;
	...
	output = new_buf();
	bprintlnf(output, CTAG(_OLCBORDER) "%s{x",
			  stringf(ch, 0, Center, "=",
					  "[ " CTAG(_OLCVAR) "%s{x : " CTAG(_OLCVAL) "%s"
					  CTAG(_OLCBORDER) " ]", ed->name,
					  olc_ed_vnum(ch->desc)));
	show_struct(ch, output, ed->type_zero, ed->table, ch->desc->pEdit, -1);
	bprintlnf(output, CTAG(_OLCBORDER) "%s{x", draw_line(ch, "=", 0));
...
AreaData *get_olc_area(Descriptor * d)
{
...
void olcedit(CharData * ch, const char *argument, OlcTable * table)
{
	AreaData *pArea;
...
const char *olc_ed_name(Descriptor * d)
{
	OlcTable *ed;
...
const char *olc_ed_name_long(CharData * ch)
{
	OlcTable *ed;
...
void olc_msg(CharData * ch, const char *n_fun, const char *fmt, ...)
{
	char buf[MPL];
	va_list args;
...
AreaData *get_area_data(int vnum)
{
	AreaData *pArea;
...
ChannelData *get_chan_data(int chan)
{
...
SongData *get_song_data(int song)
{
...
SkillData *get_skill_data(int skill)
{
...
GroupData *get_group_data(int group)
{
...
ClassData *get_class_data(int Class)
{
...
ColorData *get_color_data(int color)
{
...
bool check_vnum(vnum_t vnum, int type)
{
	int i;
...
void display_resets(CharData * ch)
{
	RoomIndex *pRoom;
	ResetData *pReset;
	CharIndex *pMob = NULL;
	...
	for (pReset = pRoom->reset_first; pReset; pReset = pReset->next)
	{
		...
		switch (pReset->command)
		{
			...
			case 'M':
				...
				{
					chprintlnf(ch, "Load Mobile - Bad Room %ld",
							   pReset->arg3);
					continue;
				}
			...
			case 'O':
				...
				{
					chprintlnf(ch, "Load Object - Bad Object %ld",
							   pReset->arg1);
					continue;
				}
				...
				if (!(pRoomIndex = get_room_index(pReset->arg3)))
				{
					chprintlnf(ch, "Load Object - Bad Room %ld",
							   pReset->arg3);
					continue;
				}
			...
			case 'P':
				if (!(pObjIndex = get_obj_index(pReset->arg1)))
				{
					chprintlnf(ch, "Put Object - Bad Object %ld",
							   pReset->arg1);
					continue;
				}
				...
				{
					chprintlnf(ch, "Put Object - Bad To Object %ld",
							   pReset->arg3);
					continue;
				}
			...
			case 'E':
				if (!(pObjIndex = get_obj_index(pReset->arg1)))
				{
					chprintlnf(ch, "Give/Equip Object - Bad Object %ld",
							   pReset->arg1);
					continue;
				}
			...
			case 'F':
				...
				chprintlnf(ch, "R[%5ld] %s door of %-19.19s reset to %s",
			...
			case 'R':
				...
				{
					chprintlnf(ch, "Randomize Exits - Bad Room %ld",
							   pReset->arg1);
					continue;
				}
...
void add_reset(RoomIndex * room, ResetData * pReset, int pIndex)
{
	ResetData *reset;
	int iReset = 0;
	...
	if (is_number(arg1))
	{
		...
		else
		{
			cmd_syntax(ch, NULL, n_fun, "<number> OBJ <vnum> <wear_loc>",
					   ...
					   "<number> DELETE", "<number> RANDOM [#x exits]",
...
Do_Fun(do_alist)
{
	...
	while (--i >= 0)
	{
		bprintlnf(output,
				  ...
				  areas[i]->builders, flag_string(area_flags,
												  areas[i]->area_flags));
	}
...
void edit_start(CharData * ch, void *OLC, ed_t Ed)
{
	ed_t old_ed;
	OlcTable *t;
...
void autoset(CharIndex * pMob)
{
	...
	ac_n =
		80 - (level * (7.77 + (level / 90))) - ((level / 9) ^ 2) -
		(bonus * 2);
	ac_x =
		80 - (level * (6.67 + (level / 90))) - ((level / 9) ^ 2) -
		(bonus * 2);
...
void autohard(CharIndex * pMob)
{
	...
	bonus = Max(1, (number_fuzzier(level) - 5));
	ac_n =
		70 - (level * (8.15 + (level / 90))) - ((level / 8) ^ 2) -
		(bonus * 2);
	ac_x =
		70 - (level * (7.12 + (level / 90))) - ((level / 8) ^ 2) -
		(bonus * 2);
	pMob->ac[AC_PIERCE] = (int) ac_n;
	pMob->ac[AC_BASH] = (int) ac_n;
	pMob->ac[AC_SLASH] = (int) ac_n;
...
void autoeasy(CharIndex * pMob)
{
	...
	bonus = Max(1, (number_fuzzier(level) - 5));
	ac_n =
		90 - (level * (7.47 + (level / 90))) - ((level / 10) ^ 2) -
		(bonus * 2);
	ac_x =
		90 - (level * (6.37 + (level / 90))) - ((level / 10) ^ 2) -
		(bonus * 2);
	pMob->ac[AC_PIERCE] = (int) ac_n;
	pMob->ac[AC_BASH] = (int) ac_n;
	pMob->ac[AC_SLASH] = (int) ac_n;
...
void autoweapon(ObjIndex * pObj)
{
	...
	mod =
		Max(1,
			number_range(number_fuzzier((int) bonus) * 6 / 10,
...
void area_set_auto(AreaData * pArea)
{
	ObjIndex *pObjIndex;
	CharIndex *pMobIndex;
	vnum_t vnum;
...
Do_Fun(do_areaset)
{
	...
	else if (ch->in_room != NULL && (pArea = ch->in_room->area) != NULL)
	{
		...
		cmd_syntax(ch, NULL, n_fun, "world       - set the whole world.",
				   "            - set your current area.", NULL);

	return;
}

void find_area_levels(CharData * ch, AreaData * pArea, bool set)
{
	...
	if (!set)
		chprintlnf(ch, "Recomended levels for %s: %03d - %03d", pArea->name,
				   low, high);
	else
	{
...
Do_Fun(do_arecommend)
{
	...
	if (NullStr(arg) || (set = !str_cmp(arg, "set")))
	{
		...
		chprintlnf(ch, "Use '%s <area> [set]' to see a specific area.",
				   n_fun);
	}
...
Do_Fun(do_relevel)
{
	...
	for (vnum = parea->min_vnum; vnum < parea->max_vnum; vnum++)
	{
		...
		if ((pobj = get_obj_index(vnum)) != NULL && pobj->level > 0)
		{
			if (pAdd)
				pobj->level =
					Range(1, pobj->level + amount, MAX_MORTAL_LEVEL);
			else if (pSub)
				pobj->level =
					Range(1, pobj->level - amount, MAX_MORTAL_LEVEL);
	...
	if (!str_cmp(argument, "in"))
	{
				   ...
				   capitalize(ch->in_room->area->name));
		{
			for (area = area_first; area; area = area->next)
			{
				...
				{
					for (vnum = area->min_vnum; vnum <= area->max_vnum;
						 vnum++)
					{
						if ((room = get_room_index(vnum)) != NULL)
							...
							{
								if (room->exit[i] != NULL)
									if (room->exit[i]->u1.to_room->area ==
										target)
									{
			...
			if ((room = get_room_index(vnum)))
			{
				for (i = 0; i < MAX_DIR; i++)
					if (room->exit[i] != NULL)
						...
						{
							chprintlnf(ch, "%-5s from %7ld [%s]", dir_name[i],
									   vnum,
...
Do_Fun(do_afun)
{
	vinterpret(ch, n_fun, argument, "links", do_alinks, "set", do_areaset,
			   "relevel", do_relevel, "recommend", do_arecommend, NULL,
			   do_afun_help);
}
