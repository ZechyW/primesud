...
void show_flag_cmds(CharData * ch, FlagTable * flag_table)
{
	int flag;
	Column Cd;
...
void show_skill_cmds(CharData * ch, tar_t tar)
{
	int sn;
	Column Cd;
...
bool show_help_structure(CharData * ch, const void *structure,
...
bool check_range(vnum_t lower, vnum_t upper)
{
	AreaData *pArea;
	int cnt = 0;
...
AreaData *get_vnum_area(vnum_t vnum)
{
	AreaData *pArea;
...
bool check_reset_exit(RoomIndex * pRoom, int door)
{
	ResetData *pReset;
...
bool change_exit(const char *n_fun, CharData * ch, const char *argument,
				 int door)
{
	RoomIndex *pRoom;
...
Olc_Fun(redit_oreset)
{
	...
	{
		cmd_syntax(ch, NULL, n_fun, "<vnum> <args>", "-no_args = into room",
				   "-<obj> = into obj", "-<mob> <wear_loc> = into mob", NULL);
		return false;
	}
	...
	if (NullStr(arg2))
	{
		...
		olc_msg(ch, n_fun, "%s (%ld) has been loaded and added to resets.",
				capitalize(pObjIndex->short_descr), pObjIndex->vnum);
	}
	else if (NullStr(argument)
			 && ((to_obj = get_obj_list(ch, arg2, pRoom->content_first)) !=
				 NULL))
	{
		pReset = new_reset();
		pReset->command = 'P';
...
const char *show_obj_values(ObjIndex * obj)
{
	...
	switch (obj->item_type)
	{
		...
		case ITEM_STAFF:
			sprintf(buf,
					...
					"[v3] Spell:          %s", obj->value[0], obj->value[1],
					obj->value[2],
					obj->value[3] !=
		...
		case ITEM_PILL:
			sprintf(buf,
					"[v0] Level:  [%ld]" NEWLINE "[v1] Spell:  %s" NEWLINE
					"[v2] Spell:  %s" NEWLINE "[v3] Spell:  %s" NEWLINE
		...
		case ITEM_WEAPON:
			sprintf(buf,
					"[v0] Weapon class:   %s" NEWLINE
					"[v1] Number of dice: [%ld]" NEWLINE
					"[v2] Type of dice:   [%ld]" NEWLINE
					"[v3] Type:           %s" NEWLINE
					"[v4] Special type:   %s", flag_string(weapon_types,
														   obj->value[0]),
		...
		case ITEM_CONTAINER:
			sprintf(buf,
					...
					"[v1] Flags:      [%s]" NEWLINE "[v2] Key:     %s [%ld]"
					NEWLINE "[v3] Capacity    [%ld]" NEWLINE
		...
		case ITEM_DRINK_CON:
			sprintf(buf,
					...
					"[v1] Liquid Left:  [%ld]" NEWLINE "[v2] Liquid:       %s"
					NEWLINE "[v3] Poisoned:     %s", obj->value[0],
		...
		case ITEM_FOUNTAIN:
			sprintf(buf,
					...
					"[v2] Liquid:    	    %s", obj->value[0], obj->value[1],
					liq_table[obj->value[2]].liq_name);
			break;

		case ITEM_FOOD:
			sprintf(buf,
					"[v0] Food hours: [%ld]" NEWLINE "[v1] Full hours: [%ld]"
					NEWLINE "[v3] Poisoned:   %s", obj->value[0],
...
bool set_obj_values(CharData * ch, ObjIndex * pObj, int value_num,
					const char *argument)
{
			...
			switch (value_num)
			{
				...
				case 2:
					if (atol(argument) != 0)
					{
						...
						if (get_obj_index(atov(argument))->item_type !=
							ITEM_KEY)
						{
...
bool set_value(CharData * ch, ObjIndex * pObj, const char *argument,
			   int value)
{
	if (NullStr(argument))
	{
...
bool oedit_values(CharData * ch, const char *argument, int value)
{
	ObjIndex *pObj;
...
Olc_Fun(medit_group)
{
	...
	if (!str_cmp(arg, "show") && is_number(argument))
	{
		...
		{
			for (pMTemp = char_index_hash[temp]; pMTemp;
				 pMTemp = pMTemp->next)
...
Olc_Fun(raedit_classx)
{
	...
	if (!NullStr(argument))
	{
		...
		olc_msg(ch, n_fun, "Race modifier for class %s is set to %d.", clas,
				atoi(value));
		return true;
	}
	cmd_syntax(ch, NULL, n_fun, "<class> <value>", NULL);
...
Olc_Fun(skedit_spellfun)
{
	...
	for (i = 0; spell_index[i].name != NULL; i++)
	{
		if (!str_cmp(argument, spell_index[i].name)
			|| !str_cmp(argument, spell_index[i].name + 6))
		{
...
Olc_Fun(skedit_gsn)
{
	...
	for (i = 0; gsn_index[i].name != NULL; i++)
	{
		if (!str_cmp(argument, gsn_index[i].name)
			|| !str_cmp(argument, gsn_index[i].name + 4))
		{
