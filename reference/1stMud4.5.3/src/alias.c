...
void substitute_alias(Descriptor * d, char *argument)
{
	...
	if (IsNPC(ch) || NullStr(ch->pcdata->alias[0])
	...
	{
		if (NullStr(ch->pcdata->alias[alias]))
			break;
...
Do_Fun(do_alias)
{
	...
	if (NullStr(arg))
	{
		...
		{
			if (NullStr(rch->pcdata->alias[pos])
				|| NullStr(rch->pcdata->alias_sub[pos]))
				break;
	...
	if (NullStr(argument))
	{
		for (pos = 0; pos < MAX_ALIAS; pos++)
		{
			...
			{
				chprintlnf(ch, "%s aliases to '%s'.", rch->pcdata->alias[pos],
						   rch->pcdata->alias_sub[pos]);
				return;
			}
		}
