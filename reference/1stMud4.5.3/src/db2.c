...
Boot_Fun(load_objects)
{
	...
	while (true)
	{
		...
		CheckPos(pObjIndex->item_type,
				 (item_t) flag_value(type_flags, read_word(fp)), "item_type");
		...
		switch (pObjIndex->item_type)
		{
			...
			case ITEM_FOUNTAIN:
				pObjIndex->value[0] = read_number(fp);
				pObjIndex->value[1] = read_number(fp);
				CheckPos(pObjIndex->value[2], liq_lookup(read_word(fp)),
						 "liq_lookup");
				pObjIndex->value[3] = read_number(fp);
				pObjIndex->value[4] = read_number(fp);
				break;
			...
			for (pReset = pRoom->reset_first; pReset; pReset = pReset->next)
			{
				switch (pReset->command)
				{
					...
					case 'P':
					{
						...
						ppObj->level =
							ppObj->level <
							1 ? pObjTo->level : Min(ppObj->level,
													pObjTo->level);
					}
						break;
...
void convert_object(ObjIndex * pObjIndex)
{
	int level;
	int number, type;
...
void convert_mobile(CharIndex * pMobIndex)
{
	int i;
...
char *intstr(long i, const char *word)
{
...
Do_Fun(do_memory_stats)
{
	...
	{
		chprintlnf(ch, " Running = %s (%s) Last updated: %s.", EXE_FILE,
				   fsize(chk.st_size), str_time(chk.st_mtime, GetTzone(ch),
												NULL));
	}
	else
...
#ifndef DISABLE_I3
	if (I3_is_connected())
		chprintlnf(ch, " Connected to InterMUD 3 on %s.", I3_ROUTER_NAME);
	else
		chprintln(ch, " InterMUD 3 is not currently connected.");
#endif
#ifndef DISABLE_MCCP
	chprintlnf(ch, " Total: %d - MCCP: %d - Ratio: %2.2f%%",
			   mud_info.bytes_normal, mud_info.bytes_compressed,
			   Percent((double) mud_info.bytes_compressed,
					   (double) mud_info.bytes_normal));
#endif
#ifndef DISABLE_SENDSTAT
	{
		EXTERN time_t last_sendstat_post;
		if (last_sendstat_post)
			chprintlnf(ch, " Last sendstat post was %s ago.",
					   timestr(last_sendstat_post, false));
		else
			chprintln(ch, " No stats have been posted yet.");
	}
#endif
#ifndef DISABLE_MYSQL
	chprintlnf(ch, " Mud is using a MySQL database to store data.");
#endif
...
void vinterpret(CharData * ch, const char *n_fun, const char *argument, ...)
{
...
void *index_lookup(const char *name, const struct index_type *table,
				   void *def)
{
	int i;
...
const char *index_name(void *data, const struct index_type *table,
					   const char *def)
{
	int i;
...
void fix_objprogs(void)
{
	...
	for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
	{
		...
		{
			for (list = pObjIndex->oprog_first; list != NULL;
				 list = list->next)
			{
...
void add_social(SocialData * social)
{
	SocialData *tmp;
...
void unlink_social(SocialData * social)
{
	unhash_social(social);
	UnLink(social, social, next, prev);
}

void add_help(HelpData * help)
{
	HelpData *tmp;
...
void newcmd_insert_name_sort(CmdData * a)
{
...
void add_command(CmdData * command)
{
...
void unlink_command(CmdData * command)
{
...
void hash_command(CmdData * command)
{
	int hash;
...
void unhash_command(CmdData * command)
{
	int hash;
...
void unhash_social(SocialData * social)
{
	int hash;
...
void hash_social(SocialData * social)
{
	int hash;
...
Proto(int convert_level, (const char *));

void convert_area_credits(AreaData * pArea)
{
...
int reset_door(ResetData * pReset, bool fRandom)
{
...
int open_exit(RoomIndex * toRoom, RoomIndex * pRoom, ResetData * pReset)
{
	int door;
...
void clean_temp_exit(RoomIndex * pRoom, int door)
{
	RoomIndex *pToRoom;

	if (!pRoom || (door < 0 && door >= MAX_DIR) || pRoom->exit[door] == NULL
...
void clean_entrance(RoomIndex * pRoom, ResetData * pReset)
{
	int door;
...
void add_random_exit(RoomIndex * pRoom, ResetData * pReset, bool Area)
{
	...
	for (;;)
	{
		...
		if (toRoom != NULL && !IsSet(toRoom->room_flags, ROOM_PRIVATE)
			...
			&& !IsSet(toRoom->room_flags, ROOM_NEWBIES_ONLY) && (Area
																 ||
																 !IsSet
																 (toRoom->area->area_flags,
																  AREA_CLOSED
																  |
																  AREA_PLAYER_HOMES))
			&& NullStr(toRoom->owner) && (Area || toRoom->area->clan == NULL)
...
bool check_directories(const char *exename)
{
	...
	for (i = 0;
		 strlen(directories_table[i].directory) +
...
void build_player_index(void)
{
	...
	for (i = 0; i != o; i++)
	{
		if (alist[i]->d_name[0] == '.' || !str_cmp("CVS", alist[i]->d_name)
			|| !str_cmp(alist[i]->d_name, "deleted")
			|| !str_cmp(alist[i]->d_name, "backup"))
			continue;
