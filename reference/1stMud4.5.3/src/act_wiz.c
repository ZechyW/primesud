...
Do_Fun(do_wiznet)
{
	...
	if (NullStr(arg))
	{
		...
		for (flag = 0; wiznet_table[flag].name != NULL; flag++)
		{
			...
			{
				print_on_off(&c, IsSet(ch->wiznet, wiznet_table[flag].flag),
...
void new_wiznet(CharData * ch, const void *arg1, flag_t flag, bool logit,
...
Do_Fun(do_guild)
{
	...
	if (ClanFlag(clan, CLAN_INDEPENDENT))
	{
...
Do_Fun(do_pardon)
{
	...
	if (NullStr(arg1) || NullStr(arg2))
	{
		cmd_syntax(ch, NULL, n_fun, "<character> <outlaw>", NULL);
		return;
	}
	...
	if (!str_cmp(arg2, "outlaw"))
	{
		if (IsSet(victim->act, PLR_OUTLAW))
		{
			...
			chprintln(ch, "Outlaw flag removed.");
			chprintln(victim, "You are no longer a OUTLAW.");
		}
		return;
	}
...
RoomIndex *find_location(CharData * ch, const char *arg)
{
	CharData *victim;
	ObjData *obj;
	AreaData *area;
...
Do_Fun(do_goto)
{
	...
	for (rch = ch->in_room->person_first; rch != NULL;
...
Do_Fun(do_vnum)
{
	...
	if (NullStr(arg))
	{
		cmd_syntax(ch, NULL, n_fun, "obj <name>", "mob <name>",
				   "skill <skill or spell>", NULL);
		return;
	}
...
Do_Fun(do_owhere)
{
	...
	for (obj = obj_first; obj != NULL; obj = obj->next)
	{
		...
		if (in_obj->carried_by != NULL && can_see(ch, in_obj->carried_by)
			...
			bprintlnf(buffer, "%3d) %s is carried by %s [Room %ld]", number,
			...
			bprintlnf(buffer, "%3d) %s is somewhere", number,
					  obj->short_descr);
...
const char *crs_sprintf(bool pTime, bool reason)
{
	...
	if (pTime)
	{
		sprintf(buf, "%s by %s in %s.%s", crs_type_name(crs_info.status),
				!NullStr(crs_info.who) ? crs_info.who : "System",
				format_pulse(crs_info.timer), prstr);
	}
	else
	{
		sprintf(buf, "%s by %s.%s%s", crs_type_name(crs_info.status),
				!NullStr(crs_info.who) ? crs_info.who : "System", prstr,
				crs_info.status == CRS_COPYOVER ? " Please wait....." : "");
	}
	return strip_color(buf);
}

void do_crs(const char *n_fun, CharData * ch, const char *argument,
			crs_t type)
{
	...
	if (NullStr(arg))
	{
		cmd_syntax(ch, NULL, n_fun, "now [reason]",
				   "<#value> <#mod> [reason]", "cancel [why]", "status",
				   NULL);
		chprintln(ch,
				  "If [reason] contains 'new area', the area list won't be saved.");
		return;
	}
	...
	if (!str_cmp(arg, "cancel"))
	{
		...
		announce(ch, INFO_ALL | INFO_PRIVATE, "You have cancelled the %s.",
				 n_fun);
		return;
	}
	...
	if (!str_cmp(arg, "now"))
	{
		...
		if (!NullStr(argument))
		{
			...
			if (is_name("new area", argument))
				mud_info.dont_save_area_list = true;
		}
		crs_fun(crs_info.status);
		return;
	}
...
Do_Fun(do_snoop)
{
	...
	if (victim == ch)
	{
		...
		wiznet("$N stops being such a snoop.", ch, NULL, WIZ_SNOOPS, false,
			   get_trust(ch));
		for (d = descriptor_first; d != NULL; d = d->next)
		{
...
Do_Fun(do_switch)
{
	...
	victim->color_prefix = ch->color_prefix;
	chprintln(victim, "Ok.");
	return;
}
...
Do_Fun(do_return)
{
	...
	{
		chprintlnf(ch, "You have %d tells waiting.  Type '%s' to view them.",
	...
	new_wiznet(ch->desc->original, 0, WIZ_SWITCHES, false, get_trust(ch),
			   "$N returns from %s.", GetName(ch));
...
bool obj_check(CharData * ch, ObjData * obj)
{
	if (IsTrusted(ch, GOD)
		|| (IsTrusted(ch, IMMORTAL) && obj->level <= 20 && obj->cost <= 1000)
		|| (IsTrusted(ch, DEMI) && obj->level <= 10 && obj->cost <= 500)
...
void recursive_clone(CharData * ch, ObjData * obj, ObjData * clone)
{
	ObjData *c_obj, *t_obj;

	for (c_obj = obj->content_first; c_obj != NULL;
		 c_obj = c_obj->next_content)
	{
...
Do_Fun(do_clone)
{
	...
	else if (mob != NULL)
	{
		...
		if ((mob->level > 20 && !IsTrusted(ch, GOD))
			...
			|| (mob->level > 5 && !IsTrusted(ch, DEMI)) || (mob->level > 0
															&& !IsTrusted(ch,
																		  ANGEL))
			|| !IsTrusted(ch, AVATAR))
		{
		...
		new_wiznet(ch, NULL, WIZ_LOAD, false, get_trust(ch), "$N clones %s.",
				   GetName(clone));
		return;
	}
}
...
Do_Fun(do_restore)
{
	...
	update_pos(victim);
	act("$n has restored you.", ch, NULL, victim, TO_VICT);
	new_wiznet(ch, NULL, WIZ_RESTORE, false, get_trust(ch), "$N restored %s",
			   GetName(victim));
	chprintln(ch, "Ok.");
	return;
}
...
Do_Fun(do_notell)
{
	...
	else
	{
		...
		new_wiznet(ch, NULL, WIZ_PENALTIES, true, 0, "$N revokes %s's tells.",
				   GetName(victim));
	}
...
int compare_descriptors(const void *v1, const void *v2)
{
...
Do_Fun(do_sockets)
{
	...
	bprintlnf(buf, "Desc %-5s %-12s %-15s %s (Connected)", "Port", "Name",
			  "IP Addr", "Host Name");
	...
	while (--j >= 0)
	{
		...
		bprintlnf(buf, "%-4d %-5d %-12s %-15s %s (%s)", dlist[j]->descriptor,
				  dlist[j]->port,
...
void copyover(void)
{
	...
	sprintf(arg3, "-1");
#endif
	pid = execl(EXE_FILE, EXE_FILE, "-c", arg1, arg2, arg3, (char *) NULL);

	if (pid == -1)
	{

		logf_error("copyover: execl %s", EXE_FILE);
	...
	while (letter != '#')
	{
		switch (letter)
		{
			case 'D':
				...
				if (!d_write
					(d, NEWLINE "Restoring from copyover..." NEWLINE, 0))
				{
					close_socket(d);
					break;
				}
				...
				else
				{
					...
					d_write(d, NEWLINE "Copyover recovery complete." NEWLINE,
							0);
...
int sn_spellfun_lookup(Spell_F * fun)
{
	int sn;
...
Do_Fun(do_set_help)
{
	cmd_syntax(ch, NULL, n_fun, "char <name> <option> <value>",
			   "obj <name> <option> <value>", "desc <name> <option> <value>",
...
Do_Fun(do_set)
{
	vinterpret(ch, n_fun, argument, "character", do_set_char, "mobile",
			   ...
			   "descriptor", do_set_desc, "skill", do_sset, "spell", do_sset,
			   NULL, do_set_help);
}
...
Do_Fun(do_stat_help)
{
	cmd_syntax(ch, NULL, n_fun, "char <name>", "desc <desc>", "room <name>",
...
Do_Fun(do_stat)
{
	vinterpret(ch, n_fun, argument, "char", do_stat_char, "room",
			   do_stat_room, "descriptor", do_stat_desc, "object",
			   do_stat_obj, "area", do_stat_area, NULL, do_stat_help);
}

const char *bonus_name(bonus_t type)
{
	switch (type)
	{
		case BONUS_XP:
...
void set_bonus(bonus_t status, int mod, int time, const char *format, ...)
{
	char buf[MPL];
	va_list args;
	...
	if (time != -1)
		...
		announce(NULL, INFO_MISC, "%dx %s has been activated for %s %s.",
				 mud_info.bonus.mod, bonus_name(mud_info.bonus.status),
				 intstr(mud_info.bonus.time, "minute"), mud_info.bonus.msg);

	else

		announce(NULL, INFO_MISC, "%dx %s has been activated infinitely %s.",
				 mud_info.bonus.mod, bonus_name(mud_info.bonus.status),
				 mud_info.bonus.msg);
}
...
Do_Fun(do_bonus)
{
	...
	if (NullStr(argument))
	{
		cmd_syntax(ch, NULL, n_fun, "<type (xp, qp, dam)> <mod> <time> [msg]",
				   "off", NULL);
		return;
	}
...
bool rename_char(CharData * victim, const char *name)
{
	char old_name[MSL];
	...
	if (valid_name(name))
	{
		...
		new_wiznet(NULL, NULL, 0, true, 0, "%s renamed to %s.", victim->name,
				   capitalize(name));
