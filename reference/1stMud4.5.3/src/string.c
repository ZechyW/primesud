...
void string_edit(CharData * ch, const char **pString)
{
	chprintln(ch,
			  stringf(ch, 0, Center, "{c-{C-", "{W Entering EDIT Mode ]"));
	chprintln(ch,
			  stringf(ch, 0, Center, NULL, "Type %ch on a new line for help",
					  StrEdKey(ch)));
	chprintln(ch,
...
void string_append(CharData * ch, const char **pString)
{
	chprintln(ch,
...
const char *string_replace(const char *orig, const char *old,
						   const char *pnew)
{
	char buf[STREP_SIZE];
	const char *ptr;
...
const char *string_replace_all(const char *orig, const char *old,
							   const char *pnew)
{
	const char *ptr;
...
const char *string_nocolor(const char *string)
{
	...
	switch (command)
	{
		...
		case PARSE_REPLACE_ALL:
			...
			*text = string_replace_all(*text, arg1, arg2);
			chprintlnf(ch, "All occurances of '%s' replaced with '%s'.", arg1,
					   arg2);
			break;
		case PARSE_INSERT:
			string = first_arg(string, arg1, false);
...
const char *format_string(const char *oldstring)
{
	...
	for (;;)
	{
		...
		if (*(rdesc + i - 1) != '\r')
		{
			for (i = (xbuf[0] ? (end_of_line - 1) : (end_of_line - 4)); i;
				 i--)
			{
				if (*(rdesc + i) == ' ')
					break;
...
const char *indent_string(const char *string, int indent)
{
...
const char *string_linedel(const char *string, size_t line)
{
...
const char *string_lineadd(const char *string, const char *newstr,
						   size_t line)
{
	const char *strtmp = string;
	size_t cnt = 1, tmp = 0;
...
const char *numlines(const char *string)
{
	int cnt = 1;
...
const char *replines(const char *string)
{
...
const char *hide_tilde(const char *str)
{
...
char *fill_cstr_len(const char *src, int len)
{
...
const char *stringf(CharData * ch, size_t length, align_t align, char *fill,
					const char *format, ...)
{
...
size_t skiprcol(const char *str)
{
	size_t i;
...
size_t count_codes(const char *str, char code)
{
	size_t i, cnt = 0;
...
const char *draw_line(CharData * ch, char *fill, size_t len)
{
	static char buf_new[5][MSL];
	static int i;
	char *result;
...
const char *fix_name(const char *name)
{
	...
	if ((name[0] == 'a' || name[0] == 'A') && name[1] == ' ')
		return name + 2;
	if ((name[0] == 't' || name[0] == 'T') && name[1] == 'h' && name[2] == 'e'
		&& name[3] == ' ')
		return name + 4;
...
const char *str_rep(const char *orig, const char *old, const char *pnew)
{
	...
	while ((ptr = strstr(buf, old)) != NULL && !strstr(pnew, old)
		   && (i =
...
const char *stri_rep(const char *orig, const char *old, const char *pnew)
{
	static char buf_new[5][STREP_SIZE];
	static int o;
	char *buf, rest[STREP_SIZE];
...
bool has_newline(const char *str)
{
	size_t i, j;
...
char *strcatf(char *prev, const char *next, ...)
{
	char buf[MPL];
...
char *strncatf(char *prev, size_t len, const char *next, ...)
{
	char buf[MPL];
