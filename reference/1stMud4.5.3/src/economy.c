...
{
	chprintlnf(ch,
			   "Balance : %ld gold" NEWLINE "On Hand : %ld gold, %ld silver"
			   NEWLINE "Shares  : %d gold in %d shares",
			   ch->pcdata->gold_bank, ch->gold, ch->silver,
...
Do_Fun(do_bank)
{
	...
	if (NullStr(argument))
	{
		cmd_syntax(ch, NULL, n_fun, "balance  : Displays your balance.",
		...
		if (!clan)
		{
			cmd_syntax(ch, NULL, n_fun,
					   ...
					   "buy #    : Buy # shares", "sell #   : Sell # shares",
...
#define paytype     (!QP ? "gold" : "quest points")
	...
	if (!str_prefix(arg1, "deposit"))
	{
		...
		chprintlnf(ch, "You deposit %ld %s.  Your new %sbalance is %ld %s.",
			...
			if (is_clan(ch) && clan == CharClan(ch))
			{
				...
				for (curr = mbr_first; curr != NULL; curr = curr->next)
				{
					if (!str_cmp(ch->name, curr->name))
					{
						if (!QP)
							curr->gold_donated =
								Range(0, curr->gold_donated + amount,
									  MAX_MONEY);
						else
	...
	if (!str_prefix(arg1, "withdraw"))
	{
		...
		if (clan)
		{
			if (NullStr(argument))
			{
				cmd_syntax(ch, NULL, n_fun, "withdraw <amount> qp|gold",
						   NULL);
				return;
			}
		...
		if (getbalance < amount)
		{
			chprintlnf(ch,
					   ...
					   amount, paytype, clan ? "clan " : "", getbalance,
					   paytype);
			return;
		}
		if (amount <= 0)
		...
		{
			chprintlnf(ch, "I'm sorry you can only carry %d %s!", MAX_MONEY,
					   paytype);
			return;
		}
		...
		chprintlnf(ch, "You withdraw %ld %s.  Your new %sbalance is %ld %s.",
				   amount, paytype, clan ? "clan " : "", getbalance, paytype);
		return;
	}
	...
	if (!str_prefix(arg1, "transfer"))
	{
		...
		{
			chprintlnf(ch, "Sorry %s's account can't hold that much!",
					   GetName(victim));
			return;
		}
		...
		victim->pcdata->gold_bank =
			Max(0, victim->pcdata->gold_bank + amount);
	...
	if (!str_prefix(arg1, "buy"))
	{
		...
		{
			chprintlnf(ch, "%d shares will cost you %d, deposit more money.",
		...
		ch->pcdata->shares =
			Range(0, ch->pcdata->shares + shares, MAX_SHARES);
		chprintlnf(ch,
	...
	if (!str_prefix(arg1, "sell"))
	{
		...
		if (ch->pcdata->gold_bank + (shares * mud_info.share_value) >=
			...
			ch->pcdata->gold_bank =
				Max(0,
					ch->pcdata->gold_bank + (shares * mud_info.share_value));
...
bool check_worth(CharData * ch, money_t amount, currency_t type)
{
	switch (type)
	{
		default:
