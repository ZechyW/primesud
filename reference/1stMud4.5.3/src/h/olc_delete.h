...
void unlink_reset(RoomIndex * pRoom, ResetData * pReset)
{
...
void unlink_obj_index(ObjIndex * pObj)
{
	int iHash;
...
void unlink_room_index(RoomIndex * pRoom)
{
	int iHash;
...
void unlink_char_index(CharIndex * pMob)
{
	int iHash;
...
void clean_area_links(AreaData * target)
{
	vnum_t vnum;
	char buf[20];
...
bool check_delete_struct(const char *n_fun, CharData * ch,
						 const char *argument, flag_t * flags,
						 const char *type)
{
	if (!NullStr(argument))
	{
		if (IsSet(*flags, OLC_DELETED))
		{
			chprintln(ch, "Delete cancelled.");
			RemBit(*flags, OLC_DELETED);
		}
		else
	...
	{
		chprintlnf(ch,
				   "Typing '%s' again will permanetely remove this %s!"
				   NEWLINE
				   "-or- '%s' with any argument will cancel the deletion.",
				   n_fun, type, n_fun);
		SetBit(*flags, OLC_DELETED);
		return false;
	}
...
Olc_Fun(redit_delete)
{
	...
	if (ch)
	{
		chprintlnf(ch,
				   ...
				   "descriptions and %d exits.", pIndex, rcount, edcount,
				   ecount);
		chprintlnf(ch,
...
#endif
			AreaData *pArea_next;
			crash_status crashtype = crash_info.status;
...
Olc_Fun(oedit_delete)
{
	...
	if (ch)
	{
		chprintlnf(ch, "Removed object vnum {C%ld{x and {C%d{x resets.",
				   pIndex, rcount);
...
Olc_Fun(medit_delete)
{
	...
	if (ch)
	{
		chprintlnf(ch, "Removed mobile vnum {C%ld{x and {C%d{x resets.",
				   pIndex, rcount);
		chprintlnf(ch, "{C%d{x mobiles were extracted" " from the mud.",
				   mcount);
	}
	return true;
}
...
Olc_Fun(cedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pClan->clan_flags, "clan"))
	{
		CharData *pch;
		AreaData *area;
...
Olc_Fun(songedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pSong->flags, "song"))
	{
		int i, j = 0, c;
		SongData *new_table;

		alloc_mem(new_table, SongData, top_song);

		if (!new_table)
		{
			chprintln(ch,
					  "Memory Allocation error!!! Unable to delete song.");
			return false;
		}
...
Olc_Fun(sedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pSocial->flags, "social"))
	{
...
Olc_Fun(skedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pSkill->flags, "skill"))
	{
		int i, j = 0, c;
		SkillData *new_table;

		alloc_mem(new_table, SkillData, top_skill);
...
Olc_Fun(raedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pRace->flags, "race"))
	{
		for (ihash = 0; ihash < MAX_KEY_HASH; ihash++)
		{
...
Olc_Fun(gredit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pGroup->flags, "group"))
	{
		int i, j = 0, c;
		GroupData *new_table;

		alloc_mem(new_table, GroupData, top_group);
...
Olc_Fun(dedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pDeity->flags, "deity"))
	{
...
Olc_Fun(cmdedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pCmd->flags, "command"))
	{
		unlink_command(pCmd);
		free_cmd(pCmd);
		pCmd = cmd_first;
...
Olc_Fun(cledit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pClass->flags, "class"))
	{
		int i, j = 0, c;
		ClassData *new_table;
		RaceData *race;
		...
		alloc_mem(new_table, ClassData, top_class);
...
Olc_Fun(hedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pHelp->flags, "help"))
	{
		stop_editing(pHelp);
...
Olc_Fun(coledit_delete)
{
	ColorData *pCol;

	GetEdit(ch, ColorData, pCol);

	if (check_delete_struct
		(n_fun, ch, argument, &pCol->flags, "custom color"))
	{
		int i, j = 0, c;
		ColorData *new_table;

		alloc_mem(new_table, ColorData, top_color);

		if (!new_table)
		{
			chprintln(ch,
					  "Memory Allocation error!!! Unable to delete color.");
			return false;
		}

		c = color_lookup(pCol->name);

		for (i = 0; i < top_color; i++)
			if (i != c)
				new_table[j++] = color_table[i];

		free_string(pCol->name);
		free_string(pCol->description);
		free_mem(color_table);
		color_table = new_table;
		top_color--;

		pCol = &color_table[0];
		ch->desc->pEdit = (void *) pCol;

		chprintln(ch, "Color deleted.");
	}
...
Olc_Fun(chanedit_delete)
{
	...
	if (check_delete_struct(n_fun, ch, argument, &pChan->flags, "channel"))
	{
		int i, j = 0, c, old = top_channel;
		ChannelData *new_table;
		CharData *pch;

		top_channel--;

		alloc_mem(new_table, ChannelData, top_channel);
