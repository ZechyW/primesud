...
Ed_Fun(olced_letter)
{
	...
	if (NullStr(argument))
	{
		cmd_syntax(ch, NULL, table->field, "<letter>", NULL);
		return false;
	}
	...
	*letter = *argument;
	olc_msg(ch, table->field, "'%c': Ok.", *argument);
	return true;
}
...
Ed_Fun(olced_str)
{
	...
	replace_strf(string, "%s%s", argument,
				 table->arg2 == NULL ? "" : NEWLINE);
...
void show_flags(CharData * ch, FlagTable * flag_table)
{
	Buffer *output;
	int flag;
	Column *Cd;
...
Ed_Fun(olced_flag)
{
	...
	if (stat)
	{
		...
		if (!f->settable)
		{
			olc_msg(ch, table->field, "'%s': value is not settable.",
					f->name);
			return false;
		}
...
Ed_Fun(olced_dice)
{
	...
	if ((!is_number(numb) || atoi(numb) < 1)
		|| (!is_number(type) || atoi(type) < 1) || (!is_number(bonus)
													|| atoi(bonus) < 0))
	{
		cmd_syntax(ch, NULL, table->field, syntax);
		return false;
...
Ed_Fun(olced_ed)
{
	...
	if (NullStr(command))
	{
		cmd_syntax(ch, NULL, table->field, "add [keyword]",
				   "delete [keyword]", "edit [keyword]", "format [keyword]",
				   "rename [keyword]", NULL);
		return false;
	}

	switch ((size_t) table->arg2)
	{
...
{
	return change_exit(table->field, ch, argument, (size_t) table->arg1);
}
...
Ed_Fun(olced_value)
{
	return oedit_values(ch, argument, (size_t) table->arg3);
}

bool templookup(DataTable * table, CharData * ch, const char *argument,
				void *data, int temp)
{
	int value;
...
void reset_pflags(flag_t * pflag, ProgList * pl)
{
	flag_t bitv = 0;
...
Ed_Fun(olced_delaffect)
{
	...
	{
		olc_msg(ch, table->field,
				"Only non-negative affect-numbers allowed.");
		return false;
	}
...
Ed_Fun(olced_color)
{
	colatt_t *col = (colatt_t *) data;

	if (NullStr(argument))
	{
		cmd_syntax(ch, NULL, table->field,
				   "<attribute> [foreground] [background]", NULL);
		return false;
	}

	return set_custom_color(table->field, ch, col, argument);
}
...
Ed_Fun(olced_sound)
{
	...
	if (NullStr(command))
	{
		cmd_syntax(ch, NULL, table->field, "add [file]", "remove",
				   "rename [new]", "type [cat]", "to [flag]", "volume [#]",
				   "loop [#]", "priority [#]", "restart [true/false]",
				   "url [addr]", NULL);
		return false;
	}
	...
	if (!str_cmp(command, "volume"))
	{
		...
		{
			olc_msg(ch, table->field,
					"Please keep volume between 0 and 100.");
			return false;
		}
	...
	if (!str_cmp(command, "loop"))
	{
		...
		{
			olc_msg(ch, table->field,
					"Please keep loops between -1 and 100.");
			return false;
		}
