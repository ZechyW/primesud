...
Do_Fun(do_olc_help)
{
	...
	for (i = 0; i < ED_MAX; i++)
	{
		chprintlnf(ch, "\t%s %s (%s data)", n_fun, olc_ed_table[i].longname,
				   olc_ed_table[i].name);
	}
...
Do_Fun(do_olc)
{
	...
	for (cmd = 0; cmd < ED_MAX; cmd++)
	{
		...
		{
			(*olc_ed_table[cmd].do_fun) (olc_ed_table[cmd].name, ch,
										 argument);
			return;
		}
	}
...
Do_Fun(do_redit)
{
	...
	else if (!str_cmp(arg1, "create"))
	{
		...
		if (redit_create(arg1, ch, argument))
		{
			...
			TouchArea(((RoomIndex *) ch->desc->pEdit)->area);
		}
		return;
	}
...
Do_Fun(do_cledit)
{
	...
	else if (!str_cmp(arg, "create"))
	{
		...
		{
			chprintln(ch,
					  "clEdit : Insuffecient security to create new class.");
			return;
		}
...
Do_Fun(do_chanedit)
{
	...
	if (NullStr(arg))
	{
		cmd_syntax(ch, NULL, n_fun, "create <name>", "save", "<#|name>",
				   NULL);
		return;
	}
...
Do_Fun(do_coledit)
{
	...
	if (is_number(arg))
		...
		value = color_lookup(arg);

	if (value > -1)
	{
		pCol = get_color_data(value);

		if (!pCol)
		{
			chprintln(ch, "That color does not exist.");
			return;
		}
	}
	else if (!str_cmp(arg, "save"))
	{
		rw_color_data(act_write);
		chprintln(ch, "Custom color database saved.");
		return;
	}
	else if (!str_cmp(arg, "create"))
	{
		if (ch->pcdata->security < 9)
		{
			chprintln(ch, "Insufficient security to create custom colors.");
			return;
		}
		coledit_create(arg, ch, argument);
		return;
	}
...
Do_Fun(do_cmdcheck)
{
	...
	{
		cmd_syntax(ch, NULL, n_fun, "null", "missing", "hidden", "<category",
				   "<dofun_name>", NULL);
		free_buf(b);
		free_column(Cd);
		return;
