...
Do_Fun(do_cinfo)
{
	...
	chprintlnf(ch, "Flags   : %s", flag_string(clan_flags, i->clan_flags));
	if (IsImmortal(ch))
	{
...
Do_Fun(do_clanadmin)
{
	...
	if (!str_cmp(arg1, "where"))
	{
		...
		{
			if (!IsNPC(victim) && can_see(ch, victim) && !IsSwitched(victim)
				&& is_clan(victim) && CharClan(victim) == clan)
			{
	...
	else if (!str_prefix(arg1, "members"))
	{
		...
		chprintln(ch,
				  "Lev  Name         Rank        Gold Donated QP Donated");
		chprintln(ch,
				  "--- ------------ ------------ ------------ ----------");
		for (mbr = mbr_first; mbr != NULL; mbr = mbr->next)
		{
			...
			chprintlnf(ch, "%3d %12s %12s %12ld %d", mbr->level, mbr->name,
					   mbr->clan->rank[mbr->rank].rankname, mbr->gold_donated,
					   mbr->qp_donated);
			found = true;
		}
		if (!found)
...
Do_Fun(do_join)
{
	...
	if (IsSet(ch->act, PLR_PK) && ClanFlag(ch->pcdata->invited, CLAN_NO_PK))
	{
		chprintln(ch, "You can't join a non-pk clan.");
		return;
	}
...
void update_members(CharData * ch, bool pdelete)
{
...
Do_Fun(do_roster)
{
	...
	chprintln(ch,
			  stringf(ch, 0, Center, "{W-{w=", "[ %s{W Roster ]",
					  clan->who_name));
	for (i = MAX_RANK - 1; i >= 0; i--)
	{
		...
		for (pmbr = mbr_first; pmbr != NULL; pmbr = pmbr->next)
		{
			if (pmbr->rank != i || pmbr->clan != clan || NullStr(pmbr->name))
				continue;
			sprintf(buf2, " %s%12s {W({wLvl:{R%3d{W) ", rcol[i], pmbr->name,
					pmbr->level);
			strcat(buf, buf2);
			if (++count % 2 == 0)
			{
...
Do_Fun(do_pk)
{
	...
	if (!NullStr(argument))
	{
		if (!ch->pcdata->confirm_pk)
			cmd_syntax(ch, NULL, n_fun, "with no arguments.", NULL);
		else
		{
			ch->pcdata->confirm_pk = false;
			chprintln(ch, "PK mode cancelled.");
		}
		return;
	}
	...
	if (!ch->pcdata->confirm_pk)
	{
		do_function(ch, &do_help, "PK");
		chprintlnf(ch,
				   "Type '%s' again to confirm going to {RPK{x mode for %s.",
				   n_fun, timestr(HOUR * 100, false));
		chprintlnf(ch, "Type '%s' with any argument to cancel.", n_fun);
	...
	else
	{
		...
		chprintlnf(ch, "You are now a player killer for %s.",
				   timestr(GetTimer(ch, TIMER_PK) - current_time, false));
		new_wiznet(ch, NULL, WIZ_DEATHS, true, 0,
