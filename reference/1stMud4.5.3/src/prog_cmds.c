...
void mob_interpret(CharData * ch, const char *argument)
{
	char command[MIL];
	int cmd;
...
Do_Fun(do_mpvforce)
{
	...
	for (victim = char_first; victim; victim = victim_next)
	{
		...
		if (IsNPC(victim) && victim->pIndexData->vnum == vnum && ch != victim
...
Obj_Fun(do_opechoat)
{
	...
	act(argument,
		obj->carried_by ? obj->carried_by : obj->in_room->person_first, obj,
		victim, TO_VICT);
}
...
Obj_Fun(do_opecho)
{
	...
	act(argument,
		obj->carried_by ? obj->carried_by : obj->in_room->person_first, NULL,
		NULL, TO_ROOM);
	act(argument,
		obj->carried_by ? obj->carried_by : obj->in_room->person_first, NULL,
		NULL, TO_CHAR);
					   ...
					   arg)) == NULL)
	{
						  ...
						  carried_by->in_room, arg)))
		{
			extract_obj(vobj);
				 ...
				 && (vobj =
					 get_obj_carry(obj->carried_by, arg, NULL)) != NULL)
		{
			extract_obj(vobj);
		}
		else if (obj->carried_by
				 && (vobj =
					 get_obj_wear(obj->carried_by, arg, false)) != NULL)
		{
...
Obj_Fun(do_opforce)
{
	...
	if (!str_cmp(arg, "all"))
	{
		...
		for (vch = char_first; vch != NULL; vch = vch_next)
		{
			...
			if (
				((obj->in_room && vch->in_room == obj->in_room)
				 || (obj->carried_by
...
Room_Fun(do_rptransfer)
{
	...
	if (!str_cmp(arg1, "all"))
	{
		...
		for (victim = room->person_first; victim != NULL;
			 victim = victim_next)
		{
			victim_next = victim->next_in_room;
			if (!IsNPC(victim))
