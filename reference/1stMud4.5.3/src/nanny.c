...
Nanny_Fun(HANDLE_CON_GET_TERM)
{
	switch (argument[0])
	{
		...
		case 'T':
			d_printlnf(d,
					   NEWLINE "This text should be " ESC "[%d;%dmGREEN"
					   CL_DEFAULT "." NEWLINE "This text should be " ESC
					   "[%d;%dmRED" CL_DEFAULT ".", CL_BRIGHT, FG_GREEN,
					   CL_BRIGHT, FG_RED);
...
Nanny_Fun(HANDLE_CON_GET_NEW_PASSWORD)
{
	...
	{
		d_println(d,
				  "{cPassword must be at least five characters long." NEWLINE
				  "{cPassword: {x");
		return;
	}
...
void send_race_info(Descriptor * d)
{
	RaceData *race;
...
void send_class_info(Descriptor * d)
{
	int iClass;
...
int roll_a_stat(CharData * ch, int pStat)
{
...
void init_gen_data(Descriptor * d)
{
	int i;
...
void send_deity_info(Descriptor * d)
{
	DeityData *i;
	int e;
...
void send_timezone_info(Descriptor * d)
{
	int i;
	...
	for (i = 0; i < MAX_TZONE; i++)
	{
		d_printlnf(d, "{c\t{W%5s{c - %9s - %s{x", tzone_table[i].name,
				   str_time(-1, i, "%I:%M:%S %p"), tzone_table[i].zone);
	}
	d_printlnf(d, "{W%s", draw_line(CH(d), "{w-{W-", 0));
	d_printlnf(d, "{cFor more information, use help. (i.e. 'help %s'){x",
			   tzone_table[0].name);
...
Nanny_Fun(HANDLE_CON_GET_TIMEZONE)
{
	...
	d_printlnf(d, "{cYour time zone is now %s %s (%s) {x",
			   tzone_table[i].zone, tzone_table[i].name, str_time(-1, i,
																  "%I:%M:%S %p"));
	d_println(d, NULL);

	d_println(d,
			  "{cWhat is your e-mail address? (Hit return to leave blank){x");
	d->connected = CON_GET_EMAIL;
}

Nanny_Fun(HANDLE_CON_GET_EMAIL)
{
	if (NullStr(argument))
	{
		d_println(d, "{cE-mail not set.{x");
	}
	else
	{
		char *valid;
		...
		if ((valid = is_invalid_email(argument)) != NULL)
		{
			d_printlnf(d, "{c%s{x", valid);
			d_println(d, "{cWhat is your e-mail address?{x");
			return;
		}

		replace_str(&ch->pcdata->email, argument);
		d_printlnf(d, "{cE-mail address set to '{W%s{c'.{x",
				   ch->pcdata->email);
	}
	d_println(d, NULL);
	...
	else
	{
		...
		d_println(d,
				  "{cWhat is your screen size? Please answer in the form of 'AA BB' where AA is your screen{x");
		d_println(d, "{cwidth and BB is your screen height.{x");
		d->connected = CON_GET_SCR_SIZE;
	}
}

Nanny_Fun(HANDLE_CON_GET_SCR_SIZE)
{
	int sz;
	unsigned int i, j;
	char w[MIL], h[MIL];
	...
	if (NullStr(argument))
	{
		d_printlnf(d, "{cDefault screen size of %d by %d used.{x",
				   DEFAULT_SCR_WIDTH, DEFAULT_SCR_HEIGHT);
		init_gen_data(d);
		d->connected = CON_ROLL_STATS;
		return;
	...
	argument = one_argument(argument, w);

	if (!is_number(w) || (sz = atoi(w)) < 10 || sz > 250)
	{
		d_println(d,
	...
	argument = one_argument(argument, h);

	if (!NullStr(h))
	{
		if (!is_number(h) || (sz = atoi(h)) < 10 || sz > 100)
		{
			d_println(d,
					  "{cPlease keep your screen height between 10 and 100 lines.{x");
			return;
		}
		else
		{
	...
	d_println(d,
			  stringf(ch, 0, Center, "{?-{W=", "{c[ %d Columns by %d Lines ]",
					  d->scr_width, d->scr_height));
	for (i = 0; i < d->scr_height; i++)
	{
		d_print(d, "{?*");
		for (j = 0; j < 6; j++)
			d_printf(d, "{?%*s", ScrWidth(d) / 6, "*");
		d_println(d, NULL);
	}
	d_println(d, draw_line(ch, "{`-{W=", 0));
	d_println(d,
			  "{cBased on the above page, is your screen size set to your satisfaction? ({WY{c/{WN{c) ");
	d->connected = CON_CONFIRM_SCR_SIZE;
}

Nanny_Fun(HANDLE_CON_CONFIRM_SCR_SIZE)
{
	switch (toupper(argument[0]))
	{
		case 'Y':
			d_printlnf(d,
					   "{cScreen size set to {W%d{c columns by {W%d{c lines.{x",
					   d->scr_width, d->scr_height);
			init_gen_data(d);
			d->connected = CON_ROLL_STATS;
			break;
...
void send_weapon_info(Descriptor * d)
{
	CharData *ch = CH(d);
	int i;
...
Nanny_Fun(HANDLE_CON_READ_MOTD)
{
	...
	wiznet("$N has left real life behind.", ch, NULL, WIZ_LOGINS, false,
		   get_trust(ch));
	...
	{
		d_printlnf(d,
				   "{R==> {WThere were {R%s {Wmade to the mud today type {G%s{W to view.{x",
				   intstr(i, "change"), cmd_name(do_changes));
	}
	unfinished_quest(ch);
	update_explored(ch);
	checkcorpse(ch);
...
void nanny(Descriptor * d, const char *argument)
{
	...
	switch (d->connected)
	{
		...
		default:
			...
			NANNY_CASE(CON_CONFIRM_NEW_PASSWORD,
			...
			NANNY_CASE(CON_GET_EMAIL, HANDLE_CON_GET_EMAIL);

			NANNY_CASE(CON_GET_SCR_SIZE, HANDLE_CON_GET_SCR_SIZE);

			NANNY_CASE(CON_CONFIRM_SCR_SIZE, HANDLE_CON_CONFIRM_SCR_SIZE);
