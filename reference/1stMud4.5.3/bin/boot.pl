...
# Here's how to do it...
#
# 1) Make sure to edit the boot.pl file and insert your values at
#    the top.
#
...
#
#    */6 * * * * /home/user/1stMud/bin/boot.pl
#
...
# v 1.2, Jun 14, 2004   Updated for 1stMud - Markanth			#
#########################################################################
#########################################################################
...
$home		=	"/home/user/1stMud";
$exec       =   "$home/bin/1stMud";
...
# Shutdown file, use to disable starting up.
$shutdown	=	"$home/data/shutdown.txt";
...
sub restart_mud {
        ...
        $timet = time();
        chdir $startPath;
        system ("$exec --startup-script $port > $logdir/$timet.log 2>&1 &");
        return;
}
#########################################################################
