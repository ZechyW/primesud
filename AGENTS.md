...
Treat `tml.py` as stable reusable infrastructure. Do not change its public API or add game logic to it.

## Constraints and pitfalls

1. **HP Prime Python is not CPython.** Many stdlib modules missing, built-ins have reduced method sets. Consult **[BUILTINS.md](BUILTINS.md)** when unsure whether a feature exists.
...
7. **Use `str()` + concatenation in persisted/serialized strings.** Physical HP Prime Python has a heap-sensitive string formatting bug; avoid `%` there. Transient UI strings may still use `%`. See `PRIME_STRING_FORMAT_BUG.md`.
